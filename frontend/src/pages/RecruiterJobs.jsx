...
export default function RecruiterJobs() {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await jobService.delete(deleteTarget._id || deleteTarget.id);
      setJobs(jobs.filter((j) => (j._id || j.id) !== (deleteTarget._id || deleteTarget.id)));
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8 animate-fade-in">
      ...
      {error && <ErrorMessage message={error} onRetry={fetchJobs} className="mb-4" />}
      ...
      {loading ? (
        ...
        <div className="card flex flex-col items-center justify-center py-16 text-center">
          ...
          <Link to="/recruiter/jobs/create" className="btn-primary mt-6">
            <Plus className="h-4 w-4" />
            Post New Job
          </Link>
        </div>
        ...
        <div className="space-y-4">
          {jobs.map((job) => (
            <div key={job._id || job.id} className="card-hover p-5 animate-slide-up">
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Link
                    to={`/recruiter/jobs/${job._id || job.id}/applications`}
                    className="btn-secondary text-xs px-3 py-2"
